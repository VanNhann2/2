...
export const violationRouter = (router) => {
  ...
  router.post('/public/violations', async (req, res, next) => {
    try {
      ...
      const startDate = ''
      const endDate = ''
      const plateOnly = null
      const { plate, page } = req.body
      const { platform } = req.query

      if (!validator.verifyPlate(plate)) {
        throw new RequestError({ code: StatusCodes.BAD_REQUEST, message: 'Biển số không hợp lệ' })
      }
  ...
  router.post('/violations/statistical/exportReport', async (req, res, next) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `Bao_cao_thong_ke_${validator.defineTimeline(timeline)}_${
          timeline === 'day'
